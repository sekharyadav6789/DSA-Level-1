...
public class Main {
  ...
  public static class LinkedList {
    ...
    public static int addTwoListsHelper(Node one,int pv1,Node two, int pv2,LinkedList res){
        //Base Case- when both one and two reaches null
        if(one==null && two==null){
            return 0;
        }
        int data=0;
        //if one has more digits than two
        if(pv1>pv2){
            int oc=addTwoListsHelper(one.next,pv1-1,two,pv2,res);
            data=one.data+oc;
        }
        //if two has more digits than one
        else if(pv1<pv2){
            int oc=addTwoListsHelper(one,pv1,two.next,pv2-1,res);
            data=two.data+oc;
        }//if one and two are on the same digit
        else{
            int oc=addTwoListsHelper(one.next,pv1-1,two.next,pv2-1,res);
            data=one.data+two.data+oc;
        }
        //adding number in the resultant linkedlist and return the carry
        int nd=data%10;
        int nc=data/10;
        res.addFirst(nd);
        return nc;
    ...
    public static LinkedList addTwoLists(LinkedList one, LinkedList two) {
      ...
      //If there is any last carry then add in the resultant linkedlist
      if(oc>0){
          res.addFirst(oc);
      }
      return res;
